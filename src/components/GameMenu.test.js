...
describe("GameMenu Component", () => {
  ...
  it("renders flag count and timer elements", () => {
    ...
    expect(container.querySelector(".MineSweeper__time")).toHaveTextContent("0"); // timer starts at 0
  });
  ...
  it("resets timer when gameStatus is 'notPlaying'", () => {
    ...
    expect(container.querySelector(".MineSweeper__time")).toHaveTextContent("3");

    rerender(<GameMenu {...baseProps} gameStatus="notPlaying" />);
    expect(container.querySelector(".MineSweeper__time")).toHaveTextContent("0");
  });

  it("increments timer every second when gameStatus is 'playing'", () => {
    const { container } = render(<GameMenu {...baseProps} gameStatus="playing" />);

    act(() => {
      vi.advanceTimersByTime(5000);
    });
    expect(container.querySelector(".MineSweeper__time")).toHaveTextContent("5");
  });
